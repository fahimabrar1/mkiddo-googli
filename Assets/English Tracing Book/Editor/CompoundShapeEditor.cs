...
public class CompoundShapeEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var correspondingObjectFromSource = PrefabUtility.GetCorrespondingObjectFromSource(compoundShape.gameObject);

        EditorGUILayout.Separator();
        #if !(UNITY_5 || UNITY_2017 || UNITY_2018_0 || UNITY_2018_1 || UNITY_2018_2)
                //Unity 2018.3 or higher
                if (compoundShapeAlreadyExits && correspondingObjectFromSource != null)
                {
        ...
        {
            shapesManagers = GameManager.FindObjectOfType<SingletonManager>().GetShapesManagers(null, true);
            ...
            if (compoundShapeAlreadyExits)
            {
                for (int i = 0; i < shapesManagers.Count; i++)
                {
                    if (correspondingObjectFromSource!=null && shapesManagers[i].shapes.Where(x => x.prefab != null).Select(x => x.prefab.GetInstanceID()).Where(x => x == correspondingObjectFromSource.GetInstanceID()).Any())
                    {
    ...
    private void CreateCompoundShape(CompoundShape compoundShape)
    {
        ...
        PrefabUtility.ApplyPrefabInstance(GameObject.Find(shapesManagers[selectedShapesManager].name), InteractionMode.AutomatedAction);

        EditorSceneManager.OpenScene("Assets/English Tracing Book/Scenes/Game.unity");

        CompoundShape[] csc = FindObjectsOfType<CompoundShape>();
        foreach (CompoundShape cs in csc)
        {
            DestroyImmediate(cs.gameObject);
        }

        EditorUtility.DisplayDialog("Successful Message", $"Scentence/Word is created sucessfully in the {shapesManagers[selectedShapesManager].name} gameobject in the Main scene", "ok");
    }
}